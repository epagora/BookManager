...
public class DatabaseAdapter {
    ...
    public Cursor search(String dbTable, String[] columns, String column, int id) {
        return db.query(dbTable, columns, "column = ?", new String[]{"id"}, null, null, null);
    }
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE TABLE_A(_ID_A INTEGER PRIMARY KEY AUTOINCREMENT, NAME_A TEXT)");
            //作品テーブル（作品コード[主キー]、作品名、著者コード[外部キー]）
            db.execSQL("CREATE TABLE TABLE_W(_ID_W INTEGER PRIMARY KEY AUTOINCREMENT, TITLE_W TEXT, A_ID_W INTEGER, FOREIGN KEY(A_ID_W) REFERENCES TABLE_A(_ID_A) ON DELETE CASCADE)");
            //巻数テーブル（作品コード[主キー、外部キー]、巻数[主キー]、所有、既読
            db.execSQL("CREATE TABLE TABLE_B(W_ID_B INTEGER, NUMBER_B TEXT, BOUGHT_B INTEGER, READ_B INTEGER, PRIMARY KEY(W_ID_B, NUMBER_B), FOREIGN KEY(W_ID_B) REFERENCES TABLE_W(_ID_W) ON DELETE CASCADE)");
